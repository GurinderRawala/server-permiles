...
async function addDriver(driverRepo, log, driver, callback) {
  try {
    ...
    return callback(null, {
      msg: `${driver.firstname} has been added as a driver`,
    })
  } catch (err) {
    log.error(err)
    return callback(err)
...
async function inviteDriver(
  ...
  const inviteDriver = await Driver.createInviteDriver(driver, files, {
    clock,
    uploadService,
    token,
  })
  ...
  mailer.send('invite-driver.hbs', {
    to: inviteDriver.email,
    subject,
    payload,
  })
...
async function activateDriverAccount(
  ...
  const driverPayload = await Driver.createActiveDriverAccount(driver, {
    hashingService,
    log,
  })
...
async function resetPasswordDriver(
  driverRepo,
  log,
  mailer,
  token,
  ...
  const driverInfo = await getDriverByEmail(
    driverRepo,
    log,
    driver,
    (err, info) => {
...
module.exports = {
  ...
  createUpdateDriver: ({ driverRepo, log }) =>
    updateDriver.bind(null, driverRepo, log),
  createGetDriver: ({ driverRepo, log }) =>
    getDriverById.bind(null, driverRepo, log),
  createGetDriverByEmail: ({ driverRepo, log }) =>
    getDriverByEmail.bind(null, driverRepo, log),
  createActiveDriverAccount: ({ driverRepo, log, hashingService }) =>
    activateDriverAccount.bind(null, driverRepo, log, hashingService),
  createResetPasswordDriver: ({ driverRepo, log, mailer, token }) =>
    resetPasswordDriver.bind(null, driverRepo, log, mailer, token),
}
