...
const {
  createAddUserAccount,
  createUpdateUserAccount,
  createActivateUserAccount,
  createSignUpUserAccount,
  createSigninUserAccount,
  createResetPasswordUserAccount,
...
exports.registerRoutes = (server, modules) => {
  ...
  const validationUpdateUserAccount = userAccountValidation(
    'user-account:update'
  )
  router.post(
    '/user-accounts/update-user-account',
    [
      sessionHandler,
      determineUserRole,
      permissionUpdateUserAccount,
      validationUpdateUserAccount,
    ],
    (req, res, next) => {
      const { validationErrorMessage } = validation
      if (validationErrorMessage(req, res, next)) return
  ...
  const validationActivateUserAccount = userAccountValidation(
    'user-account:activate'
  )
  router.post(
    '/user-accounts/activate',
    [
      sessionHandler,
      determineUserRole,
      permissionActivateUserAccount,
      validationActivateUserAccount,
    ],
    (req, res, next) => {
      const { validationErrorMessage } = validation
      if (validationErrorMessage(req, res, next)) return
  ...
  const validateResetPassword = userAccountValidation(
