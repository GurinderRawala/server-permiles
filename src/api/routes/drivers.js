...
const {
  ...
  createUpdateDriver,
  createGetDriver,
  createGetDriverByEmail,
  createActiveDriverAccount,
  createResetPasswordDriver,
} = require('../../driver')
...
exports.registerRoutes = (server, modules) => {
  ...
  const validateInviteDriver =
    validation.driverRoutesValidation('driver:invite')
  router.post(
  ...
  const validateGetDriver =
    validation.driverRoutesValidation('driver:get-driver')
  router.get(
    '/drivers/:email',
  ...
  router.get('/drivers/:id', [validateDriverExits], (req, res, next) => {
    const { validationErrorMessage } = validation
    if (validationErrorMessage(req, res, next)) return
  ...
  const validationActivateDriver =
    validation.driverRoutesValidation('driver:activate')
  router.post(
    '/drivers/activate',
  ...
  const validationResetPasswordDriver = validation.driverRoutesValidation(
    'driver:reset-password'
  )
  router.post(
    '/drivers/reset-password',
    [validationResetPasswordDriver],
    (req, res, next) => {
      if (validation.validationErrorMessage(req, res, next)) return
