...
exports.truckRoutesValidation = (
  ...
  switch (route) {
    case 'truck:add-truck':
      return [
        ...
        check('vinNumber')
          ...
          .isLength({ min: 17 })
          .custom((vinNumber) =>
            truckRepo.findOne({ where: { vinNumber } }).then((truck) => {
              if (truck) {
    ...
    case 'truck:update-truck':
      return [
        check('id', 'uuid is required')
          ...
          .isUUID()
          .custom((id) =>
            truckRepo.findByPk(id).then((truck) => {
              if (!truck) {
    ...
    case 'truck:by-truck-number':
      return [
        check('truckNo', 'Truck Number is required')
          .trim()
          .notEmpty()
          .if(body('truckNo').notEmpty())
          .custom((truckNo) =>
            truckRepo.findOne({ where: { truckNo } }).then((truck) => {
              if (!truck) {
