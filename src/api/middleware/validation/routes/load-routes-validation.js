...
exports.loadRoutesValidation = (
  ...
  switch (route) {
    ...
    case 'loads:update-load':
      return [
        check('id', 'Invalid id for load')
          .trim()
          .isUUID()
          .custom((value) =>
            loadRepo.findByPk(value).then((load) => {
              if (!load) {
    ...
    case 'loads:by-loadId':
      return [
        check('loadId', 'Load number is required')
          .trim()
          .notEmpty()
          .custom((loadId) =>
            loadRepo.findOne({ where: { loadId } }).then((load) => {
              if (!load) {
