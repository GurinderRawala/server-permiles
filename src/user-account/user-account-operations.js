...
async function activateUserAccount(
  ...
  const updatedUser = await UserAccount.createActiveUserAccount(account, {
    log,
    hashingService,
  })
...
async function signUpUserAccount(
  ...
  if (error) {
    return callback({
      msg: 'Token has been expired, contact support to signup.',
    })
  }
...
async function findUserAccountByEmail(userAccountRepo, log, account, callback) {
  ...
  const res = await userAccountRepo.findOne({
    where: { email: account.email },
  })
  log.info({ res }, 'User information')
  return callback(null, res)
}
async function signinUserAccount(
  userAccountRepo,
  log,
  hashingService,
  token,
  ...
  const user = await findUserAccountByEmail(
    userAccountRepo,
    log,
    account,
    (err, res) => {
  ...
  const verify = await UserAccount.createSigninUserAccount(payload, {
    hashingService,
    log,
  })
  if (!verify) {
  ...
  const accessToken = token.create(
    {
      ...
      driverId: '7242a987-4b2a-4d7b-aff5-827ae028bd98',
    },
    { expiresIn: '48h' }
  )
...
async function resetPasswordUserAccount(
  userAccountRepo,
  log,
  mailer,
  token,
  ...
  const userInfo = await findUserAccountByEmail(
    userAccountRepo,
    log,
    user,
    (err, info) => {
...
module.exports = {
  createAddUserAccount: ({ userAccountRepo, log }) =>
    addUserAccount.bind(null, userAccountRepo, log),
  createUpdateUserAccount: ({ userAccountRepo, log }) =>
    updateUserAccount.bind(null, userAccountRepo, log),
  createGetUserRoleById: ({ userAccountRepo, log }) =>
    getUserRoleById.bind(null, userAccountRepo, log),
  createActivateUserAccount: ({ userAccountRepo, log, hashingService }) =>
    activateUserAccount.bind(null, userAccountRepo, log, hashingService),
  createGetUserAccountById: ({ userAccountRepo, log }) =>
    getUserAccountById.bind(null, userAccountRepo, log),
  createSignUpUserAccount: ({ userAccountRepo, log, token }) =>
    signUpUserAccount.bind(null, userAccountRepo, log, token),
  createSigninUserAccount: ({ userAccountRepo, log, hashingService, token }) =>
    signinUserAccount.bind(null, userAccountRepo, log, hashingService, token),
  createResetPasswordUserAccount: ({ userAccountRepo, log, mailer, token }) =>
    resetPasswordUserAccount.bind(null, userAccountRepo, log, mailer, token),
}
