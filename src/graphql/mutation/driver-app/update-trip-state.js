...
const updateTripState = ({ log, tripRepo, ...modules }) => ({
  responseToTrip: {
    ...
    resolve: async (_, args, ctx) => {
      ...
      log.info(
        { trip, args, ctx: ctx.driverId, id: args.id },
        'Driver trip information'
      )

      if (trip[0] === 1) {
        const modifiedTripOutput = resolveWithModifiedTripOutput({
          log,
          tripRepo,
          ...modules,
        })
        const res = await modifiedTripOutput(
          _,
          { ...args, where: { ...args.where, id: args.id } },
          ctx
        )
        return res[0]
      }
