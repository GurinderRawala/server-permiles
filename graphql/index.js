...
module.exports.registerGraphQL = (server, modules) => {
  ...
  server.use(
    '/api/graphql',
    [sessionHandler, determineUserRole, permissionsGraphQL, uuidMiddleware],
    graphqlHTTP((req) => ({
      schema,
      context: req,
      pretty: true,
  ...
  const driverAppQuery = require('./query').registerDriverAppGraphQL(
    server,
    modules
  )
  const driverAppMutation = require('./mutation').registerDriverMutation(
    server,
    modules
  )
  ...
  server.use(
    '/driver-app/graphql',
    [sessionHandler, determineUserRole, uuidMiddleware],
    graphqlHTTP((req) => ({
