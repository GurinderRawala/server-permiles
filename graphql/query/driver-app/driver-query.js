...
function resolveWithModifiedTripOutput({ tripRepo, loadRepo, log }) {
  return async function (_, args, ctx) {
    try {
      ...
      const assignedTrips = await tripRepo.findAll(
        createWhereCondition(ctx, mergeForDriverIdCondition(ctx, args))
      )
      const response = await Promise.all(
        assignedTrips.map(async (trip) => {
          const tripInfoList = trip.tripInfo.map(mapToJSONparse)
          const loads = await Promise.all(
            tripInfoList.map(async ({ id }) => {
...
const driverInfoGraphQL = ({ driverRepo }) => ({
  driverInfo: {
    ...
    resolve: async (_, __, context) =>
      await driverRepo.findByPk(context.driverId),
  },
})
...
const loadsGraphQL = ({ loadRepo }) => ({
  findAssignedLoads: {
    ...
    resolve: async (_, args, ctx) =>
      await loadRepo.findAll(
        createWhereCondition(ctx, mergeForDriverIdCondition(ctx, args))
      ),
  },
})

const mergeForDriverIdCondition = (ctx, args) =>
  merge({}, args, { where: { assignedTo: ctx.driverId, ...args.where } })
