...
async function inviteUser(
  ...
  const inviteUser = UserAccount.createInviteUser(user, client, {
    clock,
    token,
  })
  ...
  if (response) {
    ...
    callback(null, {
      msg: `${inviteUser.role} has been Invited to join ${payload.company}`,
    })
  }
}

module.exports = {
  createAddClient: ({ clientRepo, log }) =>
    addClient.bind(null, clientRepo, log),
  createUpdateClient: ({ clientRepo, log }) =>
    updateClient.bind(null, clientRepo, log),
  createGetClient: ({ clientRepo, log }) =>
    getClientById.bind(null, clientRepo, log),
  createInviteUser: ({
    clientRepo,
    userAccountRepo,
    log,
