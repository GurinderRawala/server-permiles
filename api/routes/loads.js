...
exports.registerRoutes = (server, modules) => {
  ...
  const validateUpdateLoad =
    validation.loadRoutesValidation('loads:update-load')
  router.post(
    '/loads/update-load',
    [
      sessionHandler,
      determineUserRole,
      permissionsUpdateLoad,
      validateUpdateLoad,
    ],
    (req, res, next) => {
      if (validation.validationErrorMessage(req, res, next)) return
  ...
  const validateGetLoadByLoadId =
    validation.loadRoutesValidation('loads:by-loadId')
  router.post(
    '/loads/by-loadId',
    [
      sessionHandler,
      determineUserRole,
      permissionsGetLoadByLoadId,
      validateGetLoadByLoadId,
    ],
    (req, res, next) => {
      if (validation.validationErrorMessage(req, res, next)) return
