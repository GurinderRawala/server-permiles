  ...
  switch (route) {
    case 'trip:add-trip':
      return [
        ...
        check('assignedTo', 'Assigned driver id is required')
          .trim()
          .isUUID()
          .custom((value) =>
    ...
    case 'trip:update-trip':
      return [
        check('id', 'Invalid id for trip')
          .trim()
          .isUUID()
          .custom((value) =>
            tripRepo.findByPk(value).then((trip) => {
              if (!trip) {
    ...
    case 'trip:by-tripId':
      return [
        check('tripId', 'Trip number is required')
          .trim()
          .notEmpty()
          .custom((tripId) =>
            tripRepo.findOne({ where: { tripId } }).then((trip) => {
              if (!trip) {
