...
const validateEmailExits = (check, brokerRepo) => {
  return check('email', 'Provide Broker email address')
    .isEmail()
    .custom((value) =>
      brokerRepo.findOne({ where: { email: value } }).then((broker) => {
        if (broker) {
...
const validateBrokerId = (check, brokerRepo, field) => {
  return check(field, 'Broker Id is required')
    .trim()
    .isUUID()
    .custom((value) =>
      brokerRepo.findByPk(value).then((broker) => {
        if (!broker) {
...
const brokerRoutesValidation = ({ check }, brokerRepo, clientRepo, route) => {
  switch (route) {
    ...
    case 'broker:update-broker':
      return [validateBrokerId(check, brokerRepo, 'id')]
    case 'broker:by-name':
      return [
        check('name', 'Broker Name is Required')
          .trim()
          .notEmpty()
          .custom((name) =>
            brokerRepo
