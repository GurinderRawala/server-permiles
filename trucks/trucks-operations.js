...
async function createTruck(
  ...
  const truckPayload = await Truck.uploadTruckFiles(truck, files, {
    uploadService,
  })
...
async function updateTruck(truckRepo, log, truck, callback) {
  try {
    ...
    return res[0] === 1
      ? callback(null, { msg: `Truck has been updated` })
...
module.exports = {
  createAddTruck: ({ truckRepo, log, uploadService }) =>
    createTruck.bind(null, truckRepo, log, uploadService),
  createUpdateTruck: ({ truckRepo, log }) =>
    updateTruck.bind(null, truckRepo, log),
  createGetTruckById: ({ truckRepo, log }) =>
    getTruckById.bind(null, truckRepo, log),
  createGetTruckList: ({ truckRepo, log }) =>
    getTruckList.bind(null, truckRepo, log),
  createGetTruckByTruckNumber: ({ truckRepo, log }) =>
    getTruckByTruckNumber.bind(null, truckRepo, log),
}
