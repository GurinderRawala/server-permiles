...
async function createTrailer(
  ...
  const truckPayload = await Trailer.uploadTrailerFiles(trailer, files, {
    uploadService,
  })
...
async function updateTrailer(trailerRepo, log, trailer, callback) {
  try {
    const res = await trailerRepo.update(trailer, {
      where: { id: trailer.id },
    })
    return res[0] === 1
      ? callback(null, { msg: `Trailer has been updated` })
...
async function getTrailerByTrailerNumber(trailerRepo, log, trailer, callback) {
  try {
    ...
    if (res.length === 0) {
      return callback({
        msg: `No trailer found by trailer#: ${trailer.trailerNo}`,
      })
    }
    return callback(null, res)
  } catch (err) {
...
module.exports = {
  createAddTrailer: ({ trailerRepo, log, uploadService }) =>
    createTrailer.bind(null, trailerRepo, log, uploadService),
  createUpdateTrailer: ({ trailerRepo, log }) =>
    updateTrailer.bind(null, trailerRepo, log),
  createGetTrailerById: ({ trailerRepo, log }) =>
    getTrailerById.bind(null, trailerRepo, log),
  createGetTrailerList: ({ trailerRepo, log }) =>
    getTrailerList.bind(null, trailerRepo, log),
  createGetTrailerByTrailerNumber: ({ trailerRepo, log }) =>
    getTrailerByTrailerNumber.bind(null, trailerRepo, log),
}
